...
class MFCF:
    ...
    def generate_outline_box(
        ...
        for i in trange(0, len(infos), desc=f"generate_outline_box: iterating over infos"):
        ...
        print("Tracking with TrackSmooth")
        tracker = TrackSmooth(self.dataset_cfg.GeneratorConfig)
        tracker.tracking(all_labels, all_pose)

        for i in trange(0, len(infos), desc="Generating output infos from MFCF"):
